...
public class UserController {
	...
	public ModelAndView addUser(HttpServletRequest request, HttpServletResponse response)
		...
		user.setSex("保密");
		user.setImage("img/logo2.png");
		user.setBirthday(Tools.createDate("yyyy-MM-dd", "1970-01-01"));
		user.setDate(new Date());
		user.getLocalCreateTime();
		user.setStatus(0);
		result = userService.addUser(user);
	...
	public ModelAndView userInfoPage(HttpServletRequest request, HttpServletResponse response) {
		...
		int numD = diaryService.selectByUserID(user.getID()).size();
		mov.addObject("user", user);
		mov.addObject("numF", numF);
		mov.addObject("numD", numD);
	...
	public ModelAndView deletUser(HttpServletRequest request, HttpServletResponse response) {
		...
		diaryService.deleteByUserID(id);
		return new ModelAndView("redirect:/usermanagerpage");
	}
	...
	@RequestMapping("/userimgpage")
	public ModelAndView userimgpage(HttpServletRequest request, HttpServletResponse response)
			throws UnsupportedEncodingException {
		ModelAndView mov = new ModelAndView("userimg");
	...
	public ModelAndView upimg(MultipartFile file, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		// 保存数据库的路径
		String sqlPath = null;
		// 定义文件保存的本地路径
		String localPath = "C:\\Users\\44977\\Workspaces\\kaoyanbao\\src\\main\\webapp\\static\\img\\";
		// 定义 文件名
