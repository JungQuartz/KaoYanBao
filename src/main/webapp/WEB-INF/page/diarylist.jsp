...
<body class=" theme-blue"  style="background-color:white;">
			...
			<div class="ProfileMain-header">
				<ul role="tablist" class="Tabs ProfileMain-tabs">
					<li role="tab" class="Tabs-item Tabs-item--noMeta"
						aria-controls="Profile-activities">我的日记</li>
					</ul>
	...
	<div class="main-content">
		<table class="table table-striped table-bordered">
			...
			<tbody>
				<c:forEach items="${diarys}" var="d" varStatus="st" begin="0">
					<tr>
						<td><a href="diarypage?ID=${d.ID}" target="_blank">${d.title}</a></td>
						<td><fmt:formatDate value="${d.date}" pattern='yyyy-MM-dd HH:mm:ss'/></td>
						<td>
							<button type="button" class="btn btn-danger"
								onclick="window.location.href='deletediary?ID=${d.ID}'"
		...
		<div style="text-align: center">
			<a href="?start=0">首 页</a> <a
				href="?start=${LP.start-LP.count}">上一页</a> <a
				href="?start=${LP.start+LP.count}">下一页</a> <a
				href="?start=${LP.last}">末 页</a>
		</div>
